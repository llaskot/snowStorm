...
class TestCriticalPath(BaseTest):
    ...
    def test_check_flights_values(self):
        ...
        assert flights_page.get_current_url().__contains__(saved.flights_url), "wrong URL"
        assert flights_page.get_origin().__contains__(saved.flying_from_code), \
            "Origin field doesn't contain airport code: " + saved.flying_from_code
        assert flights_page.get_from() == saved.month + str(val.from_date), \
            "Wrong from date"
        assert flights_page.get_to() == saved.month + str(val.to_date), \
            "Wrong to date"
        assert flights_page.get_adults() == str(val.adults), \
            "Wrong adults number"
        assert flights_page.get_children() == str(val.children), \
        ...
        assert hotel_search_page.get_current_url().__contains__(saved.hotels_url), "wrong URL"
        assert hotel_search_page.get_from_date() == saved.month + str(val.from_date), \
            "Wrong from date"
        assert hotel_search_page.get_adults_num() == val.adults, \
            "Wrong adults number"
        assert hotel_search_page.get_children_num() == val.children, \
            "Wrong children number"
        ...
        assert room_page.get_current_url().__contains__(saved.rooms_url), "wrong URL"
        room_page.click_close_banner_btn()
        assert room_page.get_room_info(1).get('per_person') == saved.hotel_info.get('per_person'), \
            "wrong per person price of the cheapest room"
        assert room_page.get_room_info(1).get('total') == saved.hotel_info.get('total'), \
        ...
        assert payment_page.get_current_url().__contains__(saved.payment_url), "wrong URL"
        ...
        assert package_inf["airport_code"] == saved.flying_from_code, "Wrong departure airport code in package info"
        assert package_inf['hotel_name'] == saved.hotel_info['hotel_name'], "Wrong hotel name in package info"
        assert package_inf["date_from"] == saved.month + str(val.from_date), "Wrong from date in package info"
        assert package_inf["date_to"] == saved.month + str(val.to_date), "Wrong to date in package info"
        assert hotel_inf['hotel_name'] == saved.hotel_info['hotel_name'], "Wrong hotel name in hotel info"
        assert hotel_inf["details"] == saved.room_info['details'], "Wrong room details in hotel info"
        assert hotel_inf['date_from'] == saved.month + str(val.from_date), "Wrong from date in hotel info"
        assert hotel_inf['date_to'] == saved.month + str(val.to_date), "Wrong to date in hotel info"
        assert hotel_inf['adults'] == val.adults, "Wrong number of adults in hotel info"
        assert hotel_inf['children'] == val.children, "Wrong number of children in hotel info"
        assert flights_inf['departure_code'] == saved.flying_from_code, "Wrong departure airport code in flights info"
        assert flights_inf['adults'] == val.adults, "Wrong number of adults in flights info"
        assert flights_inf['children'] == val.children, "Wrong number of children in flights info"
        for key in saved.departure_flights_info.keys():
            assert flights_inf['departure_flights_info'][key] == saved.departure_flights_info[key],\
                "Wrong "+key+" value in departure in flights info "
        for key in saved.return_flights_info.keys():
            assert flights_inf['return_flights_info'][key] == saved.return_flights_info[key],\
                "Wrong "+key+" value in departure in flights info "
